...
@section('content')
    @canany(['showManager','showResponsibleForDrivers'])
        <section id="query-list-wrapper" class="invoice-list-wrapper mw-100">
            <div class="card">
                <div class="card-datatable table-responsive">
                    ...
                    <div class="row m-0" id="custom_filters">
                        @can('showLogist')
                            @if(request()->sent)
                                <label class="col-12">Выгрузка</label>
                                <div class="col-12 mb-1">
                                    <a class="btn btn-primary mt-1 mt-lg-0 mt-md-0"
                                       href="{{route('exportTodaySentQueriesExcel')}}" target="_blank"
                                       style="width: 200px;">
                                        Выгрузка за сегодня
                                    </a>
                                    <a class="btn btn-primary mt-1 mt-lg-0 mt-md-0"
                                       href="{{route('exportTomorrowSentQueriesExcel')}}" target="_blank"
                                       style="width: 200px;">
                                        Выгрузка на завтра
                                    </a>
                                </div>
        ...
        let queryDataTable = new Vue({
            ...
            watch: {
                'filter': {
                    handler: function (key) {
                        ...
                        setCookie('fastDateFilter', key.fastDateFilter, 365 * 24 * 3600 * 1000)
                    },
                    deep: true,
                }
            },
            methods: {
                initialState() {
                    return {
                        ...
                        filter: {
                            fastDateFilter: (getCookie('fastDateFilter') !== undefined) ? getCookie('fastDateFilter') : "today",
                            startDate: "",
                            endDate: "",
                        }
                    }
                ...
                initDataTable: function () {
                    $('#query_list_table').DataTable({
                        ...
                        drawCallback: function (settings) {
                            ...
                            setTimeout(function () {
                                $('#query_list_table_wrapper td').addClass('dtr-control');
                            }, 50);
                        },
                        searchDelay: 500,
                        stateSave: true,
            ...
            mounted() {
                ...
                $(document).on('click', '.query-preview-btn', function () {
                    ...
                    queryDataTable.queryModalInit($(this).data('id'));
                });
                $(document).on('click', '.query-copy-btn', function () {
                    queryDataTable.deleteQuery($(this).data('id'));
                });
                /*Jquery Events For Rendered Elements*/
            }
        });
